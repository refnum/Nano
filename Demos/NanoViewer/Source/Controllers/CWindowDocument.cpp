...
{	EventTypeSpec		theEvents[] = {	{ kEventClassWindow, kEventWindowBoundsChanged },
	...
	mImageView->SetZoomLevels(MAKE_VECTOR(float, kZoomLevels));
	...
	if (!HasFile())
		...
		if (theImage.IsValid())
			...
			SetFile(appBundle.GetResource(kImageExample));
		}
...
{	const NFile		*theFile = (const NFile *) msgData;
	...
	switch (theMsg) {
		case kMsgImageReceivedFile:
			...
			SetFile(*theFile);
			SetImage(mImageView->GetImage(), false);
			break;
...
{	HISize		zoomedSize;
	...
	NMenuUtilities::SetItemMark(NULL, kCmdDocumentAutoSize, autoSize);
...
//----------------------------------------------------------------------------
OSStatus CWindowDocument::SetImage(const NCGImage &theImage, bool setModified)
{
