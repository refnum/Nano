...
{	OSStatus			theErr;
	...
	switch (hiCmd.commandID) {
		...
		case kCmdDocumentSetShapeCircle:
			theErr = SetShape(hiCmd.commandID);
			break;
...
//----------------------------------------------------------------------------
#pragma mark -
OSStatus CWindowDocument::SetShape(UInt32 theCmd)
{
	...
	switch (theCmd) {
		case kCmdDocumentSetShapeTriangle:
			mShapeView->SetShape(kShapeTriangle);
			break;

		case kCmdDocumentSetShapeRectangle:
			mShapeView->SetShape(kShapeRectangle);
			break;

		case kCmdDocumentSetShapeCircle:
			mShapeView->SetShape(kShapeCircle);
			break;
...
void CWindowDocument::SetColor(void)
{
	...
	// Update the color
	mShapeView->SetFillColor(mToolbar->GetColorPicker()->GetColor());
