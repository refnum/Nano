...
//============================================================================
//		CApplication::CommandDemoNotify : Display a user notification.
//----------------------------------------------------------------------------
OSStatus CApplication::CommandDemoNotify(void)
{	static UInt32		sMsgCount = 1;
	NString				theText;



	// Show the message
	theText.Format("This is message %d. Try invoking this command again to queue up another notification...", sMsgCount);
	sMsgCount++;

	NUserNotification::GetInstance()->PostMessage(theText);
