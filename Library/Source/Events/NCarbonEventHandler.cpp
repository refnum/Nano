...
{	OSStatus	theErr;
	...
	if (mEventHandler != NULL)
		{
		theErr = RemoveEventHandler(mEventHandler);
...
{	OSStatus	theErr;
	...
	if (!theEvents.empty())
		...
		// Update our state
		mEvents.insert(mEvents.end(), theEvents.begin(), theEvents.end()); 
...
{	bool		isDead, *oldFlag;
	...
	// Update commands
	//
	// Events are typically processed through DoEventXXXX methods, however to simplify
	// command status updating we provide an alternative API for our sub-classes.
	if (theErr == eventNotHandledErr)
		{
		if (theEvent.IsClass(kEventClassCommand) && theEvent.IsKind(kEventCommandUpdateStatus))
			theErr = UpdateCommandStatus(theEvent);
		}
...
DEFAULT_EVENT_METHOD(DoEventCommandUpdateStatus)
...
//============================================================================
//		NCarbonEventHandler::UpdateCommandStatus : Update a command's status.
//----------------------------------------------------------------------------
OSStatus NCarbonEventHandler::UpdateCommandStatus(NCarbonEvent &theEvent)
{	CommandStatus		theStatus;
	...
	if (hiCmd.attributes & kHICommandFromMenu)
		...
		if (theStatus == kCommandEnable)
			EnableMenuCommand( hiCmd.source.menu.menuRef, hiCmd.commandID);

		else if (theStatus == kCommandDisable)
			DisableMenuCommand(hiCmd.source.menu.menuRef, hiCmd.commandID);
		}
