...
{	NString			theKey, theValue;
	...
	if (theHeaders != NULL)
		...
		numItems = theKeys.GetSize();
		
		for (n = 0; n < numItems; n++)
			{
			theKey   = theKeys.GetValueString(n);
			theValue = theHeaders->GetValueString(theKey);

			CFHTTPMessageSetHeaderFieldValue(cfRequest, theKey, theValue);
			}
		}
...
{	NString				appName, appVersion, macPlatform;
	...
	theResult.Format("%s/%s (Mac OS %d.%d.%d; %dMhz; %dMb; %s)",
						appName.GetUTF8(),
						appVersion.GetUTF8(),
						(sysVersion >> 16) & 0xFF,
						(sysVersion >>  8) & 0xFF,
