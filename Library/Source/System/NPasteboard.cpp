...
{	PasteboardRef	thePasteboard;
	...
	InitializeSelf(thePasteboard, true);
}
...
{	PasteboardRef	thePasteboard;
	...
	InitializeSelf(thePasteboard, false);
}
...
{	UInt32				n, numFlavors;
	...
	for (n = 0; n < numFlavors && nsColor == NULL; n++)
		...
		if (theFlavor == kUTNTypeColor)
			{
			theData  = GetItemData(theItem, theFlavor);
			nsColor  = (NSColor *) [NSKeyedUnarchiver unarchiveObjectWithData:theData];
...
{	UInt32			n, numFlavors;
	...
	for (n = 0; n < numFlavors && !theImage.IsValid(); n++)
		...
		if (theFlavor.ConformsTo(kUTNTypeImage))
			{
			theData  = GetItemData(theItem, theFlavor);
			theImage.Set(NCoreGraphicsUtilities::CreateImageFromData(theData, theFlavor));
			}
		}
...
{	NData	textUTF16, textUTF8;
	...
	// Prepare the values
	if (!textUTF16.SetData(theValue.GetSizeBytes(kCFStringEncodingUTF16), theValue.GetUTF16()) ||
		! textUTF8.SetData(theValue.GetSizeBytes(kCFStringEncodingUTF8),  theValue.GetUTF8()))
		return;
...
{	NString		theText;
	...
	if (!theData.SetData(theText.GetSizeBytes(), theText.GetUTF8()))
		return;
...
//============================================================================
//		NPasteboard::GetValuesText : Get the text values.
//----------------------------------------------------------------------------
NStringList NPasteboard::GetValuesText(void) const
{	UInt32			n, numItems;
	...
	for (n = 0; n < numItems; n++)
		{
		theValue = GetValueText(GetItem(n));
...
//============================================================================
//		NPasteboard::GetValuesFile : Get the file values.
//----------------------------------------------------------------------------
NFileList NPasteboard::GetValuesFile(void) const
{	UInt32			n, numItems;
	NFileList		theValues;
	NFile			theValue;
	...
	for (n = 0; n < numItems; n++)
		{
		theValue = GetValueFile(GetItem(n));
		if (theValue.IsValid())
			theValues.push_back(theValue);
		}
...
//----------------------------------------------------------------------------
#pragma mark -
void NPasteboard::InitializeSelf(PasteboardRef thePasteboard, bool takeOwnership)
{
	...
	Set(thePasteboard, takeOwnership);
}
