...
{	int		pipeStdIn[2], pipeStdOut[2], pipeStdErr[2];
	...
	// buffered output that that the caller has still to read.
	//
...
{	va_list		argList;
	...
	SetArguments(NTextUtilities::CollectArguments(argList, arg1));
	va_end(argList);
}
...
void NTask::WriteInput(const NString &theValue)
{
	...
	if (mStdIn != PIPE_INVALID)
		write(mStdIn, theValue.GetUTF8(), theValue.GetSizeBytes());
}
...
{	bool		mainThread;
	...
	if (arg1 != NULL)
		{
		va_start(argList, arg1);
		theTask.SetArguments(NTextUtilities::CollectArguments(argList, arg1));
		va_end(argList);
		}
	...
	// our timer, so we need to check the status manually by polling.
	mainThread = NThreadUtilities::IsMainThread();
	...
	while (theTask.IsRunning())
		...
		if (mainThread)
			NSystemUtilities::DispatchCurrentEventLoop();
		else
			{
			NThread::Sleep();
			theTask.CheckStatus();
