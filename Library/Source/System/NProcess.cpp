...
//============================================================================
//		NProcess::Compare : Compare two objects.
//----------------------------------------------------------------------------
#pragma mark -
CFComparisonResult NProcess::Compare(const NComparable &theObject) const
{	const NProcess		*theProcess = dynamic_cast<const NProcess*>(&theObject);
	pid_t				pid1, pid2;
	Boolean				areSame;
	OSStatus			theErr;
	...
	NN_ASSERT(theProcess != NULL);



	// Compare the processes
	//
	// Althoughs processes have equality, they do not have any real ordering.
	theErr = SameProcess(&mPSN, &theProcess->mPSN, &areSame);
	NN_ASSERT_NOERR(theErr);

	if (theErr == noErr && areSame)
		return(kCFCompareEqualTo);
	
	pid1 = (pid_t) *this;
	pid2 = (pid_t) *theProcess;
	
	return(GET_CF_COMPARE(pid1, pid2));
}
