...
CFTypeRef NCFObject::CreateDuplicate(CFTypeRef cfObject)
{
...
#endif
    static CFTypeID kCFTypeBitVector    = CFBitVectorGetTypeID();
	...
	if (theType == kCFTypeURL)
		cfCopy = (CFTypeRef) CFURLCreateWithString(kCFAllocatorNano, CFURLGetString((CFURLRef) cfObject), NULL);
	
	else if (theType == kCFTypeBitVector)
		cfCopy = (CFTypeRef) CFBitVectorCreateMutableCopy(kCFAllocatorNano, 0, (CFBitVectorRef) cfObject);
    
	else
		{
		cfCopy = CFPropertyListCreateDeepCopy(kCFAllocatorNano, cfObject, kCFPropertyListMutableContainersAndLeaves);
