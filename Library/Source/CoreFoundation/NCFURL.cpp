...
NCFURL::NCFURL(const NString &theString)
{
	...
	// Initialize ourselves
	InitializeSelf(CFURLCreateWithString(kCFAllocatorNano, NTextUtilities::EncodeURL(theString), NULL));
}
...
NCFURL::NCFURL(const NData &theData, CFStringEncoding theEncoding)
{
	...
	// Initialize ourselves
	InitializeSelf(CFURLCreateWithBytes(kCFAllocatorNano, theData.GetData(), theData.GetSize(), theEncoding, NULL));
}
...
NCFURL::NCFURL(const NFile &theFile)
{
	...
	// Initialize ourselves
	InitializeSelf(CFURLCreateWithFileSystemPath(kCFAllocatorNano, theFile.GetPath(), kCFURLPOSIXPathStyle, false));
}
...
{	static CFURLRef		sDefaultURL = CFURLCreateWithString(kCFAllocatorNano, CFSTR("file://localhost/"), NULL);



	// Get the value
	//
	// Unfortunately empty CFURLs are not allowed, so we need to use a
	// non-empty value for the default case.
	return(sDefaultURL);
}
...
//============================================================================
//		NCFURL::InitializeSelf : Initialize ourselves.
//----------------------------------------------------------------------------
#pragma mark -
void NCFURL::InitializeSelf(CFURLRef cfURL)
{
