...
{	UInt64		theSize, numRead;
	...
	NN_ASSERT(theSize <= ULONG_MAX);



	// Resize the buffer
	if (!theData.SetSize((UInt32) theSize))
		return(theData);
	...
	theData.SetSize((UInt32) numRead);

	NN_ASSERT_NOERR(theErr);
	NN_ASSERT(numRead <= ULONG_MAX);
...
//----------------------------------------------------------------------------
OSStatus NFileUtilities::SetFileText(const NFile &theFile, const NString &theText, CFStringEncoding theEncoding)
{
...
{	UInt64		theSize, numWritten;
	...
	// Write the data
	theErr = mutableFile.SetForkSize(0);
...
//============================================================================
//		NFileUtilities::GetFileName : Get a file name.
//----------------------------------------------------------------------------
NString NFileUtilities::GetFileName(const NString &baseName, const NUTI &theUTI)
{	NString		fileName;



	// Get the file name
	fileName.Format("%@.%@", baseName, theUTI.GetFileExtension());
...
{	FSRef		theFSRef;
    ...
    NN_ASSERT(theErr == noErr || theErr == fnfErr);
    
    if (theErr == noErr)
        theFile.SetFSRef(theFSRef, fileName);
...
//============================================================================
//      NFileUtilities::GetVolumeList : Get the list of mounted volumes.
//----------------------------------------------------------------------------
FSVolumeRefNumList NFileUtilities::GetVolumeList(void)
{	FSVolumeRefNumList		theResult;
	...
	do
		{
		theErr = FSGetVolumeInfo(kFSInvalidVolumeRefNum, theIndex, &vRefNum, kFSVolInfoNone, NULL, NULL, NULL);
		if (theErr == noErr)
			{
			theResult.push_back(vRefNum);
			theIndex++;
			}
		}
	while (theErr == noErr);
...
//============================================================================
//		NFileUtilities::GetVolumeRoot : Get the root of a volume.
//----------------------------------------------------------------------------
NFile NFileUtilities::GetVolumeRoot(FSVolumeRefNum vRefNum)
{	FSRef		theFSRef;
	NFile		theRoot;
	OSStatus	theErr;
	...
	theErr = FSGetVolumeInfo(vRefNum, 0, NULL, kFSVolInfoNone, NULL, NULL, &theFSRef);
	if (theErr == noErr)
		theRoot.SetFSRef(theFSRef);
...
{	UInt32				n, numItems;
	...
	va_start(argList, arg1);
        ...
        append(theArgs, NTextUtilities::CollectArguments(argList, arg1));
...
//============================================================================
//      NFileUtilities::ResolveAliases : Resolve aliases.
//----------------------------------------------------------------------------
void NFileUtilities::ResolveAliases(NFileList &theFiles)
{	NFileListIterator	theIter;



	// Resolve the aliases
	for (theIter = theFiles.begin(); theIter != theFiles.end(); theIter++)
		theIter->ResolveAlias();
}
...
//============================================================================
//		NFileUtilities::SetOpenFileLimit : Set the open file limit.
//----------------------------------------------------------------------------
OSStatus NFileUtilities::SetOpenFileLimit(UInt32 theValue)
{	struct rlimit		theLimit;
	...
	// Set the limit
	theLimit.rlim_cur = (rlim_t) theValue;
	theLimit.rlim_max = (rlim_t) theValue;
	
	sysErr = setrlimit(RLIMIT_NOFILE, &theLimit);
	NN_ASSERT_NOERR(sysErr);

	return(sysErr == 0 ? (OSStatus) noErr : (OSStatus) tmfoErr);
}
...
{	char		theBuffer[PATH_MAX];
	...
	// Get the path
	thePath = NString(getcwd(theBuffer, sizeof(theBuffer)));

	return(NFile(thePath));
}
...
{	NString			*bundleID = (NString *) theInfo.userData;
	...
	if (theBundle.IsValid() && bundleID->EqualTo(theBundle.GetIdentifier()))
		{
		stopNow = true;
		return(true);
		}
