...
//============================================================================
//		NSystemUtilities::IsMetric : Get the metric state.
//----------------------------------------------------------------------------
bool NSystemUtilities::IsMetric(void)
{	NCFObject		cfLocale;
	bool			isMetric;
	...
	if (cfLocale.Set(CFLocaleCopyCurrent()))
		isMetric = CFBooleanGetValue((CFBooleanRef) CFLocaleGetValue(cfLocale, kCFLocaleUsesMetricSystem));
	else
		isMetric = true;
...
//============================================================================
//		NSystemUtilities::DispatchCurrentEventLoop : Dispatch events.
//----------------------------------------------------------------------------
void NSystemUtilities::DispatchCurrentEventLoop(EventTime theTime)
{	EventTime			stopTime, timeLeft;
	...
	do
		{
		theErr   = ReceiveNextEvent(0, NULL, timeLeft, true, &theEvent);
		timeLeft = stopTime - GetCurrentEventTime();

		if (theErr == noErr)
			{
			theErr = SendEventToEventTarget(theEvent, theTarget);
	...
	for (n = 0; n < numParts; n++)
		...
		if (type1 == type2)
			...
			if (type1 != kPartPeriod)
				{
				theResult = part1.CompareTo(part2, kCFCompareNumerically);
...
//============================================================================
//		NSystemUtilities::SearchDictionary : Search Dictionary.app.
//----------------------------------------------------------------------------
void NSystemUtilities::SearchDictionary(const NString &theText)
{


	// Perform the search
	//
	// Dictionary.app will not perform a search if it had to be launched to
	// handle the service request (rdar://5132471).
	//
	// To work around this, we perform the service request twice - which will
	// cause the window to flicker if Dictionary.app was already running, but
	// is otherwise harmless.
	NService(kServiceSearchDictionary, theText).Execute();
	NService(kServiceSearchDictionary, theText).Execute();
}
...
//============================================================================
//		NSystemUtilities::SearchSpotlight : Search Spotlight.
//----------------------------------------------------------------------------
void NSystemUtilities::SearchSpotlight(const NString &theText)
{


	// Perform the search
	NService(kServiceSearchSpotlight, theText).Execute();
}
...
//============================================================================
//		NSystemUtilities::SearchGoogle : Search Google.
//----------------------------------------------------------------------------
void NSystemUtilities::SearchGoogle(const NString &theText)
{


	// Perform the search
	NService(kServiceSearchGoogle, theText).Execute();
}
...
//============================================================================
//		NSystemUtilities::ExecuteScript : Execute an AppleScript.
//----------------------------------------------------------------------------
OSStatus NSystemUtilities::ExecuteScript(const NString &theText)
{	NAppleScript		theScript(theText);
	OSStatus			theErr;



	// Execute the script
	theErr = theScript.Execute();
...
//----------------------------------------------------------------------------
void NSystemUtilities::DelayFunctor(const NFunctor &theFunctor, EventTime theDelay)
{	NTimer		*theTimer;
	...
	theTimer = new NTimer;
	if (theTimer != NULL)
		theTimer->AddTimer(BindFunction(NSystemUtilities::DelayedFunctor, theFunctor, theTimer), theDelay);
}
...
//----------------------------------------------------------------------------
void NSystemUtilities::DelayedFunctor(const NFunctor &theFunctor, NTimer *theTimer)
{
