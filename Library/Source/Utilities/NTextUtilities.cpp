...
{	NString		theInfo, infoData, infoRate, infoTime;
	...
	infoDone.Format(theFormat.GetUTF8(), valueFloat);
	...
	if (dataTotal == (UInt32) -1)
		...
		infoTotal.Format(theFormat.GetUTF8(), valueFloat);

		theFormat = NBundleString(kStringTransferProgressXofY, "", kNanoStrings);
		infoData.Format(theFormat.GetUTF8(), infoDone.GetUTF8(), infoTotal.GetUTF8());
		}
	...
	if (bytesPerSec != (UInt32) -1)
		...
		infoRate.Format(theFormat.GetUTF8(), valueInt);
		}
	...
	if (NMathUtilities::NotZero(timeLeft))
		...
		infoTime.Format(theFormat.GetUTF8(), valueInt);
		}
	...
	if (infoRate.IsNotEmpty())
		theInfo.Format("%s (%s)", theInfo.GetUTF8(), infoRate.GetUTF8());

	if (infoTime.IsNotEmpty())
		theInfo.Format("%s - %s", theInfo.GetUTF8(), infoTime.GetUTF8());
