...
//		Inline functions
//----------------------------------------------------------------------------
template <typename T> void vector_push_front(std::vector<T> &theVector, const T &theValue)
{
	theVector.insert(theVector.begin(), 1, theValue);
} 

template <typename T> T vector_fetch_back(std::vector<T> &theVector)
{	T	theValue;

	theValue = theVector.back();
	theVector.pop_back();
	
	return(theValue);
} 

template <typename T> void container_join(T &container1, const T &container2) 
{
