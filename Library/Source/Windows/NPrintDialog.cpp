...
// Timers
static const NTimerID kTimerDoPrint								= 1000;
...
{	Boolean		didAccept;
	...
	if (mInsideSheet)
		{
		AddTimer(kPrintDelay, kPrintDelay, kTimerDoPrint);
		return(noErr);
		}
...
//============================================================================
//		NPrintDialog::DoTimer : Handle a timer.
//----------------------------------------------------------------------------
#pragma mark -
void NPrintDialog::DoTimer(NTimerID theTimer, TimerMsg theMsg)
{


	// Handle the timer
	//
	// We use a repeating timer to handle deferred print requests.
	switch (theTimer) {
		case kTimerDoPrint:
			if (!mInsideSheet)
				{
				RemoveTimer(kTimerDoPrint);
				DoPrint();
				}
			break;
