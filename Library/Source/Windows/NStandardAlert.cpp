...
{	OSStatus	theErr;
	...
	//
	// On 10.5, GetStandardAlertDefaultParams sets the cancelButton field to 0.
	// This means that alerts that create a cancel button by supplying text must
	// also specify if they want that button to receive Escape.
	//
	// We provide a default behaviour for each button, so that creating a button
	// by supplying text will create an OK and a Cancel button by default.
	mState.defaultButton = kAlertStdAlertOKButton;
	mState.cancelButton  = kAlertStdAlertCancelButton;
}
...
{	const EventTypeSpec	theEvents[] = { { kEventClassWindow, kEventWindowSheetClosed },
	...
	if (mParent != NULL)
		...
		if (theErr == noErr)
			theErr = InstallHandler(mParent->GetHIWindow(), theEvents);
		}
...
//============================================================================
//		NStandardAlert::DoEventWindowSheetClosed : Handle kEventWindowSheetClosed.
//----------------------------------------------------------------------------
OSStatus NStandardAlert::DoEventWindowSheetClosed(NCarbonEvent &/*theEvent*/)
{
