...
class NIcon {
public:
                                        NIcon(OSType  iconType, OSType iconCreator=kSystemIconsCreator);
										NIcon(IconRef theIcon);
                                        NIcon(const NFile    &theFile, OSType iconType=0, OSType iconCreator=0);
										NIcon(const NString  &theName, const NString &subDir="", CFBundleRef theBundle=NULL);
										NIcon(const NCGImage &theImage);
