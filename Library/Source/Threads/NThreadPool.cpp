...
{	StLock	lockMutex(mLock);
	...
	// number of tasks may be queued up before any worker threads get a chance to run. 
...
//----------------------------------------------------------------------------
bool NThreadPool::WaitForTasks(EventTime waitFor)
{	bool			areDone, canTimeout;
	...
	endTime    = GetCurrentEventTime() + waitFor;
	canTimeout = NMathUtilities::NotEqual(waitFor, kEventDurationForever);
	...
	do
		...
		if (!areDone)
			...
			if (canTimeout && GetCurrentEventTime() >= endTime)
				break;
			}
		}
	while (!areDone);
