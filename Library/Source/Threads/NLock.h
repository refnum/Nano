...
public:
	...
	// Acquire/release the lock
	//
	// Mutexes are recursive, and may be called multiple times from the same thread
	// providiing every Lock is balanced with a subsequent Unlock.
...
public:
	...
	// Acquire/release the lock
	//
	// Multiple readers can acquire the lock simultaneously, but will block if a
	// thread has acquired the lock for writing.
	//
