...
static const UInt32 kSpinLockBit										= 0;
...
bool NSpinLock::SpinLockTry(UInt32 *theLock)
{
	...
	NN_ASSERT(((UInt64) theLock) % 4 == 0);
	


	// Try and acquire the lock
	return(OTCompareAndSwap8(0, 1 << kSpinLockBit, (UInt8 *) theLock));
}
...
void NSpinLock::SpinLockUnlock(UInt32 *theLock)
{
	...
	// Release the lock
	OTAtomicClearBit((UInt8 *) theLock, kSpinLockBit);
}
