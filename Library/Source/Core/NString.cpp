...
//		Public constants
//----------------------------------------------------------------------------
const NString kStringWhitespace										= "\\s";
...
//		NString::NString : Constructor.
//----------------------------------------------------------------------------
NString::NString(char theChar)
{


    // Initialize ourselves
	InitializeSelf(CFStringCreateWithBytes(kCFAllocatorNano, (const UInt8 *) &theChar, 1, kCFStringEncodingUTF8, false), true);
	...
	if (NN_DEBUG && IsEmpty() && theChar != 0x00)
		NN_LOG("Unable to convert from encoding 0X%8X [%c]", kCFStringEncodingUTF8, theChar);
}
...
NString::NString(const NData &theData, CFStringEncoding theEncoding)
{
	...
	// CFStringCreateWithBytes will fail if passed characters that are illegal
	// in the specified encoding, so we assert that a non-empty input succeeded.
	if (NN_DEBUG && theData.IsNotEmpty() && theData.GetData()[0] != 0x00 && IsEmpty())
		NN_LOG("Unable to convert from encoding 0X%8X [%s]", theEncoding, NTextUtilities::GetHexDump(theData.GetSize(), theData.GetData(), 0, true).GetUTF8());
}
...
NString::NString(const char *theString, UInt32 theLength, CFStringEncoding theEncoding)
{
	...
	// CFStringCreateWithBytes will fail if passed characters that are illegal
	// in the specified encoding, so we assert that a non-empty input succeeded.
	if (NN_DEBUG && theLength > 0 && theString[0] != 0x00 && IsEmpty())
		NN_LOG("Unable to convert from encoding 0X%8X [%s]", theEncoding, NTextUtilities::GetHexDump(theLength, theString, 0, true).GetUTF8());
}
...
//----------------------------------------------------------------------------
UInt32 NString::GetSizeBytes(CFStringEncoding theEncoding) const
{	CFIndex		theSize;
...
{	CFIndex		theSize;
	...
	// Get the data
	return((const char *) GetData(theSize, kCFStringEncodingUTF8));
}
...
{	CFIndex		theSize;
	...
	// Get the data
	return((const UniChar *) GetData(theSize, kCFStringEncodingUTF16));
}
...
//----------------------------------------------------------------------------
const UInt8 *NString::GetData(CFIndex &theSize, CFStringEncoding theEncoding) const
{	UInt32		*nullPtr;
	UInt8		*thePtr;
	...
	//
	// If we can, we return a direct pointer to the internal string data.
	theSize = GetSizeBytes(theEncoding);
...
//		NString::GetData : Get the string data.
//----------------------------------------------------------------------------
NData NString::GetData(CFStringEncoding theEncoding) const
{	CFIndex			theSize;
	const UInt8		*thePtr;
	NData			theData;
	...
	// Get the data
	thePtr = GetData(theSize, theEncoding);
	if (thePtr != NULL)
		theData.AppendData(theSize, thePtr);
...
{	CFRangeList						foundRanges;
	...
	if (MakeMutable())
		{
		// Simple replace
		if ((theFlags & kCFComparePattern) == 0)
			CFStringFindAndReplace(*this, theString, replaceWith, NormalizeRange(theRange), theFlags);
...
{	NString		matchString;
	...
	// Simple test
	if (theFlags == 0)
		isMatch = CFStringHasPrefix(*this, theString);


	// Complex test
	else
		{
		if (theFlags & kCFComparePattern)
			matchString.Format("^%@",       theString);
		else
			matchString.Format("^\\Q%@\\E", theString);

		theRange = Find(matchString, theFlags | kCFComparePattern);
		isMatch  = CFRangeIsNotEmpty(theRange);
		}
...
{	NString		matchString;
	...
	// Simple test
	if (theFlags == 0)
		isMatch = CFStringHasSuffix(*this, theString);
...
void NString::Trim(const NString &theString, bool isExact)
{
	...
	// Whitespace trim
	if (theString == kStringWhitespace)
		{
		if (MakeMutable())
...
{	int						n, errorPos, numMatches, matchStart, matchEnd, execOffset;
		...
		if (regErr > 0)
			...
			for (n = 0; n <= numMatches; n++)
				...
				if (matchStart == -1)
					...
					matchRange.location = theRange.location + execOffset + matchText.GetSize();
