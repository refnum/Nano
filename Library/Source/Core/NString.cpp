...
{	NString			tmpString, subString;
	...
	// Check our state
	//
	// We return an empty list when splitting an empty string; we test for this case up
	// front to override the default behaviour of CFStringCreateArrayBySeparatingStrings,
	// which would otherwise return a list containing a single empty string.
	if (IsEmpty())
		return(theStrings);
	...
	if (isExact)
		...
		while (tmpString.IsNotEmpty())
			{
			theRange = tmpString.Find(theString);
