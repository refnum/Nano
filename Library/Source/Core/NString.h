...
extern const NString kStringWhitespace;
...
static const CFOptionFlags kCFComparePatternNoCase				= kCFCompareCaseInsensitive | kCFComparePattern;
static const CFOptionFlags kCFCompareNumericNoCase				= kCFCompareCaseInsensitive | kCFCompareNumerically;
...
public:
	...
	// Get the size
	//
	// GetSize returns the number of characters in the string, while GetSizeBytes returns
	// the number of bytes in a particular encoding.
	UInt32								GetSize(void) const;
	UInt32								GetSizeBytes(CFStringEncoding theEncoding=kCFStringEncodingUTF8) const;
	...
	// Get the string data
	//
	// Returned pointers are to a NULL-terminated string in the specified encoding, which
	// is valid until the string object is modified or destroyed.
	//
	...
	const UInt8							*GetData(CFIndex &theSize, CFStringEncoding theEncoding=kCFStringEncodingUTF8) const;
	NData								 GetData(                  CFStringEncoding theEncoding=kCFStringEncodingUTF8) const;
...
private:
	...
	DECLARE_NSTRING_GETPOD(char,				char);
