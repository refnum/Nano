...
//----------------------------------------------------------------------------
NString NDate::GetAsString(const NString &formatString, const NString &timeZone) const
{	NCFObject	cfFormatter;
	...
	NN_ASSERT(formatString.IsNotEmpty());
	


	// Get the date
	cfFormatter = GetDateFormatter(kCFDateFormatterNoStyle, kCFDateFormatterNoStyle, timeZone);

	if (cfFormatter.IsValid())
		{
		CFDateFormatterSetFormat(cfFormatter, formatString);
...
//      NString::+= : Addition operator.
//----------------------------------------------------------------------------
const NDate& NDate::operator += (const CFGregorianUnits &theDelta)
{


    // Add the delta
	mTime = CFAbsoluteTimeAddGregorianUnits(mTime, GetTimeZone(kTimeZoneUTC), theDelta);
