...
{	static bool					sLogErrors = false;
    ...
    // variable set of views, such as an NWindowPanels), however a flag is
	// provided to debug missing views.
	//
	// If you find that a view was not bound to its member variable, this
    // flag can be set to log any missing views.
	for (theIter = mViews.begin(); theIter != mViews.end(); theIter++)
		...
		if (theErr != noErr && sLogErrors)
			...
			NN_LOG("Unable to find view { %s, %d }", NTextUtilities::FromOSType(theID.signature).GetUTF8(), theID.id);
			}
		}
}
...
//============================================================================
//		NIBWindow::ResetViews : Reset the views list.
//----------------------------------------------------------------------------
void NIBWindow::ResetViews(void)
{
