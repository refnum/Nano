...
//----------------------------------------------------------------------------
NMenu::NMenu(MenuID theID, MenuAttributes theAttributes)
{	MenuRef		theMenu;
	OSStatus	theErr;
	...
	// Create the menu
	theMenu = NULL;
	theErr  = CreateNewMenu(theID, theAttributes, &theMenu);
...
//============================================================================
//		NMenu::InsertItem : Insert an item into the menu.
//----------------------------------------------------------------------------
NMenuItem NMenu::InsertItem(MenuItemIndex		insertAfter,
							const NString		&theName,
							MenuCommand			theCmd,
							MenuItemAttributes	theAttributes)
{	NMenuItem		theItem;
	OSStatus		theErr;
	...
	NN_ASSERT(insertAfter == 0 || InRange(insertAfter));



	// Insert the item
	theErr = InsertMenuItemTextWithCFString(mMenu, theName, insertAfter, theAttributes, theCmd);
	NN_ASSERT_NOERR(theErr);

	if (theErr == noErr)
		theItem = NMenuItem(mMenu, insertAfter+1, false);
...
//============================================================================
//		NMenu::RemoveItem : Remove an item from the menu.
//----------------------------------------------------------------------------
void NMenu::RemoveItem(MenuItemIndex theIndex)
{


	// Validate our parameters
	NN_ASSERT(InRange(theIndex));



	// Remove the item
	DeleteMenuItem(mMenu, theIndex);
}
...
{	MenuItemIndex		theIndex;
	...
	if (theErr == noErr)
		theItem = NMenuItem(mMenu, theIndex, false);
