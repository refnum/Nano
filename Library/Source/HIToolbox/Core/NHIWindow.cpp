...
//============================================================================
//		NHIWindow::GetWindowClass : Get the window class.
//----------------------------------------------------------------------------
WindowClass NHIWindow::GetWindowClass(void) const
{	WindowClass		theClass;
	...
	theClass = kDocumentWindowClass;
	theErr   = ::GetWindowClass(*this, &theClass);
	NN_ASSERT_NOERR(theErr);
...
//----------------------------------------------------------------------------
void NHIWindow::SetVisible(bool isVisible, bool doFade, bool doAsync)
{
...
{	HIViewRef	hiView;
	...
	// Set the button
	theErr = SetWindowDefaultButton(*this, hiView);
	NN_ASSERT_NOERR(theErr);
}
...
{	HIViewRef	hiView;
	...
	// Set the button
	theErr = SetWindowCancelButton(*this, hiView);
	NN_ASSERT_NOERR(theErr);
}
...
//============================================================================
//		NHIWindow::GetActivationScope : Get the activation scope.
//----------------------------------------------------------------------------
WindowActivationScope NHIWindow::GetActivationScope(void) const
{	WindowActivationScope		theScope;
	OSStatus					theErr;



	// Get the activation scope
	theScope = kWindowActivationScopeNone;
	theErr   = GetWindowActivationScope(*this, &theScope);
	NN_ASSERT_NOERR(theErr);
...
//============================================================================
//		NHIWindow::SetActivationScope : Set the activation scope.
//----------------------------------------------------------------------------
void NHIWindow::SetActivationScope(WindowActivationScope theScope)
{	OSStatus	theErr;



	// Set the activation scope
	theErr = SetWindowActivationScope(*this, theScope);
	NN_ASSERT_NOERR(theErr);
}
...
{	UInt32		theFeatures;
	...
	// Get the features
	theFeatures = (UInt32) kWindowNoFeatures;
...
//============================================================================
//		NHIWindow::ConstrainPosition : Constrain the window position.
//----------------------------------------------------------------------------
void NHIWindow::ConstrainPosition(WindowRegionCode			theRegion,
									WindowConstrainOptions	theOptions,
									const HIRect			*screenRect,
									      HIRect			*structRect)
{	Rect		*ptrScreen, *ptrStruct;
	Rect		qdScreen, qdStruct;
	OSStatus	theErr;
	...
	if (screenRect != NULL)
		{
		NGeometryUtilities::ConvertRect(*screenRect, qdScreen);
		ptrScreen = &qdScreen;
		}
	...
	theErr = ConstrainWindowToScreen(*this, theRegion, theOptions, ptrScreen, ptrStruct);
	NN_ASSERT_NOERR(theErr);



	// Return the structure region
	if (structRect != NULL)
		NGeometryUtilities::ConvertRect(qdStruct, *structRect);
}
...
//============================================================================
//		NHIWindow::DoEventNAppGetUndoManager : Handle kEventNAppGetUndoManager.
//----------------------------------------------------------------------------
OSStatus NHIWindow::DoEventNAppGetUndoManager(NCarbonEvent &theEvent)
{


	// Pass the event to the application
	return(theEvent.SendToApplication());
}
