...
{	NHIObject						*theObject;
	...
	if (classID.Compare(kHIControlAdapterClassID, kCFCompareCaseInsensitive) == kCFCompareEqualTo)
		{
		...
		if (theErr == noErr && theKind.signature == kControlKindSignatureApple)
			...
			switch (theKind.kind) {
				...
				default:
					NN_ASSERT_MSG("Unknown class: %d", theKind.kind);
					break;
				}
			}
		}
	...
	for (theIter = GetFactories().begin(); theIter != GetFactories().end(); theIter++)
		{
		if (theIter->first.Compare(classID, kCFCompareCaseInsensitive) == kCFCompareEqualTo)
			{
