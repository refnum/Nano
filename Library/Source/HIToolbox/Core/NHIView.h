...
static const HIViewFeatures kHIViewFeaturesNone						= 0;
static const OptionBits     kHIViewAttributesNone					= 0;

static const float kHIViewDisabledAlpha								= 0.5f;

static const HIViewTrackingAreaID kHIViewTrackingAreaNone			= 0;
static const HIViewTrackingAreaID kHIViewTrackingAreaView			= 1;
static const HIViewTrackingAreaID kHIViewTrackingAreaCursor			= 2;


// Options
static const OptionBits kHIViewOptionsNone							= 0;
static const OptionBits kHIViewOptionIsLocked						= (1 <<  0);
static const OptionBits kHIViewOptionSetsCursor						= (1 <<  1);
static const OptionBits kHIViewOptionInvalidateOnActivate			= (1 <<  2);
static const OptionBits kHIViewOptionInvalidateOnEnable				= (1 <<  3);
static const OptionBits kHIViewOptionInvalidateOnHilite				= (1 <<  4);
static const OptionBits kHIViewOptionInvalidateOnTitle				= (1 <<  5);
static const OptionBits kHIViewOptionInvalidateOnValue				= (1 <<  6);
static const OptionBits kHIViewOptionWebViewUpdateWindowTitle		= (1 <<  7);
static const OptionBits kHIViewOptionWebViewUpdateWindowIcon		= (1 <<  8);
static const OptionBits kHIViewOptionWebViewMakeLinksExternal		= (1 <<  9);
static const OptionBits kHIViewOptionWebViewAllowContextualMenu		= (1 << 10);
static const OptionBits kHIViewOptionSearchFieldClearOnCancel		= (1 << 11);
...
// Tracking area
typedef struct {
	HIViewTrackingAreaID	theID;
	HIShapeRef				theShape;
	bool					isWithin;
	HIViewTrackingAreaRef	trackArea;
} HIViewTrackingArea;


// Lists
class NHIView;
typedef std::vector<NHIView*>										NHIViewList;
typedef NHIViewList::iterator										NHIViewListIterator;
typedef NHIViewList::const_iterator									NHIViewListConstIterator;

typedef std::vector<HIViewTrackingArea>								HIViewTrackingAreaList;
typedef HIViewTrackingAreaList::iterator							HIViewTrackingAreaListIterator;
typedef HIViewTrackingAreaList::const_iterator						HIViewTrackingAreaListConstIterator;
...
//		Class declaration
//----------------------------------------------------------------------------
class NHIView : public NHIObject {
public:
										NHIView(void);
	...
	// Get the focus state
	HIViewPartCode						GetFocus(void) const;
	...
	virtual void						ReplaceTextValue(const CFRange &theRange,  const NString &replaceWith);
	virtual void						ReplaceTextValue(const NString &theString, const NString &replaceWith, CFOptionFlags theFlags=0);
	...
	void								SetFeatures(HIViewFeatures setThese, HIViewFeatures clearThese=kHIViewFeaturesNone);
	...
	void								SetAttributes(OptionBits setThese, OptionBits clearThese=kHIViewAttributesNone);
	...
	void								SetOptions(OptionBits setThese, OptionBits clearThese=kHIViewOptionsNone);
	...
	// Get/set the display state
	bool								GetNeedsDisplay(void) const;
	void								SetNeedsDisplay(bool			isDirty  = true,
														const HIRect	*theRect = NULL,
														EventTime		theDelay = 0.0);
...
protected:
	...
	//
	// Sub-classes should call through for standard behaviour.
	...
	virtual OSStatus					DoEventNAppGetUndoManager(        NCarbonEvent &theEvent);
	...
	virtual ThemeCursor					DoDragUpdate( const NDrag &theDrag, bool isWithin);
	virtual DragActions					DoDragReceive(const NDrag &theDrag);
	...
	// Create/dispose of a tracking area
	//
	// All tracking areas are disposed of when the view is destroyed.
	void								CreateTrackingArea( HIViewTrackingAreaID theID=kHIViewTrackingAreaView, HIShapeRef theShape=NULL);
	void								DisposeTrackingArea(HIViewTrackingAreaID theID=kHIViewTrackingAreaView);


	// Get a tracking area
	HIViewTrackingArea					GetTrackingArea(HIViewTrackingAreaID theID);
...
private:
	...
	void								DisposeTrackingAreas(void);
	void								UpdateTrackingAreas( void);

	static OSStatus						TrackingHandler(EventHandlerCallRef callRef, EventRef eventRef, void *userData);
...
private:
	...
	EventTime							mTrackTime;
	HIPoint								mTrackMouse;
	
	NArray								mDraggedTypes;
	
	HIViewTrackingAreaList				mTrackingAreas;
	EventHandlerRef						mTrackingHandler;
};
