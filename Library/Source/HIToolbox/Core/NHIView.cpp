...
{	EventTypeSpec	theEvent = { kEventClassMouse, kEventMouseMoved };
	...
	//
	// This event is generated through a mouse-moved event handler, which is added
	// or removed as necessary to minimise overhead.
	needHandler = !mTrackingAreas.empty() && IsEventActive(kEventClassNControl, kEventNControlTrackingAreaWithin);
...
{	NHIView									*thisPtr = (NHIView *) userData;
	...
	// Check for a hit in this view
	theView = NULL;
	theErr  = HIViewGetViewForMouseEvent(HIViewGetRoot(theWindow), eventRef, &theView);
	NN_ASSERT_NOERR(theErr);

	if (theErr != noErr || theView != *thisPtr)
		return(eventNotHandledErr);



	// Check for a hit in a tracking area
	...
	for (theIter = thisPtr->mTrackingAreas.begin(); theIter != thisPtr->mTrackingAreas.end(); theIter++)
		{
		if (theIter->theShape == NULL || HIShapeContainsPoint(theIter->theShape, &thePoint))
			{
			// Dispatch the event
