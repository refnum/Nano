...
//============================================================================
//		NHIView::GetFocus : Get the focsu state.
//----------------------------------------------------------------------------
HIViewPartCode NHIView::GetFocus(void) const
{	HIViewPartCode		thePart;
	...
	// Get the state
	thePart = kHIViewFocusNoPart;
	theErr  = HIViewGetFocusPart(*this, &thePart);
	NN_ASSERT_NOERR(theErr);
...
void NHIView::SetCurrentValue(SInt32 theValue)
{
	...
	NN_ASSERT_MSG(theValue >= GetMinimumValue() && theValue <= GetMaximumValue(),
					"SetCurrentValue[%d] will be clamped to [%d..%d]",
					theValue, GetMinimumValue(), GetMaximumValue());
...
//============================================================================
//		NHIView::ReplaceTextValue : Replace the text value.
//----------------------------------------------------------------------------
void NHIView::ReplaceTextValue(const CFRange &theRange, const NString &replaceWith)
{	NString		theValue;



	// Replace the value
	theValue = GetTextValue();
	theValue.Replace(theRange, replaceWith);
...
//		NHIView::ReplaceTextValue : Replace the text value.
//----------------------------------------------------------------------------
void NHIView::ReplaceTextValue(const NString &theString, const NString &replaceWith, CFOptionFlags theFlags)
{	NString		theValue;
	...
	// Replace the value
	theValue = GetTextValue();
	theValue.ReplaceAll(theString, replaceWith, theFlags);
...
{	HIViewFeatures		theFeatures;
	...
	// Get the features
	theFeatures = kHIViewFeaturesNone;
...
void NHIView::SetOptions(OptionBits setThese, OptionBits clearThese)
{
	...
	// Register for events
	//
	// Some options require events to support their behaviour. Since we can't tell
	// if an event was registered for by ourselves or a sub-class, we never turn
	// off events if the option is subsequently disabled.
	//
	// Since our event handlers check the relevant option before processing (as a
	// sub-class may have registered for the event directly), this is harmless.
	if (mOptions & kHIViewOptionSetsCursor)
		{
		if (!IsEventActive(kEventClassControl, kEventControlTrackingAreaExited))
			AddEvent(      kEventClassControl, kEventControlTrackingAreaExited);

		if (   GetTrackingArea(kHIViewOptionSetsCursor).trackArea == NULL)
			CreateTrackingArea(kHIViewOptionSetsCursor);
		}
}
...
//----------------------------------------------------------------------------
NHelpTag NHIView::GetHelpTag(void) const
...
//----------------------------------------------------------------------------
void NHIView::SetHelpTag(const NHelpTag &theTag)
{	OSStatus	theErr;
	...
	// Set the help tag
	theErr = HMSetControlHelpContent(*this, theTag);
	NN_ASSERT_NOERR(theErr);
}
...
//============================================================================
//		NHIView::GetNeedsDisplay : Get the display state.
//----------------------------------------------------------------------------
bool NHIView::GetNeedsDisplay(void) const
{
...
//----------------------------------------------------------------------------
void NHIView::SetNeedsDisplay(bool isDirty, const HIRect *theRect, EventTime theDelay)
{	OSStatus	theErr;
	...
	if (!NMathUtilities::IsZero(theDelay))
		NSystemUtilities::DelayFunctor(BindSelf(NHIView::SetNeedsDisplay, isDirty, theRect, 0.0f), theDelay);



	// Set the display state
	if (theRect != NULL)
		theErr = HIViewSetNeedsDisplayInRect(*this, theRect, isDirty);
	else
		theErr = HIViewSetNeedsDisplay(      *this,          isDirty);
...
//============================================================================
//		NHIView::EncodeState : Encode the view state.
//----------------------------------------------------------------------------
OSStatus NHIView::EncodeState(NDictionary &/*theState*/) const
{


	// Encode our state
	//
	// Sub-classes should call through to the base class, then encode their state.
	return(noErr);
}
...
//============================================================================
//		NHIView::DecodeState : Decode the view state.
//----------------------------------------------------------------------------
OSStatus NHIView::DecodeState(const NDictionary &/*theState*/)
{


	// Decode our state
	//
	// Sub-classes should call through to the base class, then decode their state.
	return(noErr);
}
...
//============================================================================
//      NHIView::DoEventsChanged : Handle event changes.
//----------------------------------------------------------------------------
void NHIView::DoEventsChanged(void)
{
...
{	NPasteboard		thePasteboard(theEvent);
	...
	// Ask the base class
	//
	// We ask the base class first, so that wrappers around system controls
	// can provide default behaviour (e.g., DataBrowser column dragging).
...
{	NDrag		theDrag(theEvent);
	...
	// Receive the drag
	theDrag.SetPerformedActions(DoDragReceive(theDrag));
...
{	HISize		sizeMin, sizeMax, sizeIdeal;
	...
	// provide one either, we fall back to the "ideal size" for the view.
	//
...
{	NCarbonEvent			hitTest(kEventClassControl, kEventControlHitTest);
	HIPoint					theMouse, mouseFirst, mouseLast;
	UInt32					keyModifiers;
	...
	mouseFirst = theEvent.GetMouseLocation();
	mouseLast  = mouseFirst;
	theMouse   = mouseFirst;
	...
	// Add a click count
	//
	// Views that wish to provide different behaviour when double-clicked vs for
	// a down-drag-up sequence need a click count to distinguish these cases.
	//
	// Unfortunately kEventControlTrack does not provide this (rdar://4077394),
	// so we synthesize one by checking the position since the last track.
	theEvent.SetMouseClickCount(1);
	
	if ((theEvent.GetTime() - mTrackTime) <= NMouseUtilities::GetDoubleClickTime())
		{
		theDelta.x = fabsf(mouseFirst.x - mTrackMouse.x);
		theDelta.y = fabsf(mouseFirst.y - mTrackMouse.y);

		if (theDelta.x <= NMouseUtilities::GetDoubleClickSlop() &&
			theDelta.y <= NMouseUtilities::GetDoubleClickSlop())
			theEvent.SetMouseClickCount(2);
		}
	...
	while (DoTrackMouse(theEvent, theMouse, trackResult))
		...
		// Update the event
		theDelta.x = theMouse.x - mouseLast.x;
		theDelta.y = theMouse.y - mouseLast.y;
		mouseLast  = theMouse;
	...
	// Update our state
	mTrackTime  = GetCurrentEventTime();
	mTrackMouse = mouseFirst;
...
//============================================================================
//		NHIView::DoEventControlTrackingAreaExited : Handle kEventControlTrackingAreaExited.
//----------------------------------------------------------------------------
//		Note :	The base class does not register for this event by default,
//				but registers it when kHIViewOptionSetsCursor is enabled.
//----------------------------------------------------------------------------
OSStatus NHIView::DoEventControlTrackingAreaExited(NCarbonEvent &/*theEvent*/)
{	NCursor		theCursor;
	...
	// Reset the cursor
	//
	// Views that change the cursor should set kHIViewOptionSetsCursor, to allow
	// the base class to reset the cursor to an arrow when it leaves the view.
	if (mOptions & kHIViewOptionSetsCursor)
		theCursor.SetCursor(kThemeArrowCursor);
...
//============================================================================
//		NHIView::DoEventNAppGetUndoManager : Handle kEventNAppGetUndoManager.
//----------------------------------------------------------------------------
OSStatus NHIView::DoEventNAppGetUndoManager(NCarbonEvent &theEvent)
{	HIWindowRef		hiWindow;
	...
	if (hiWindow != NULL)
		theErr = theEvent.SendToHIWindow(hiWindow);
...
//----------------------------------------------------------------------------
ThemeCursor NHIView::DoDragUpdate(const NDrag &/*theDrag*/, bool isWithin)
{
...
//----------------------------------------------------------------------------
DragActions NHIView::DoDragReceive(const NDrag &/*theDrag*/)
{
...
//============================================================================
//      NHIView::CreateTrackingArea : Create a tracking area.
//----------------------------------------------------------------------------
void NHIView::CreateTrackingArea(HIViewTrackingAreaID theID, HIShapeRef theShape)
{	HIViewTrackingArea		theArea;
	OSStatus				theErr;



	// Validate our parameters
	if (theID != kHIViewTrackingAreaNone)
		NN_ASSERT(GetTrackingArea(theID).trackArea == NULL);



	// Create the tracking area
	theErr = HIViewNewTrackingArea(*this, theShape, theID, &theArea.trackArea);
	NN_ASSERT_NOERR(theErr);
	...
	// Update our state
	CFSafeRetain(theShape);

	theArea.theID    = theID;
	theArea.isWithin = false;
	theArea.theShape = theShape;

	mTrackingAreas.push_back(theArea);
	UpdateTrackingAreas();
}
...
//============================================================================
//      NHIView::DisposeTrackingArea : Dispose of a tracking area.
//----------------------------------------------------------------------------
void NHIView::DisposeTrackingArea(HIViewTrackingAreaID theID)
{	HIViewTrackingAreaListIterator		theIter;
	OSStatus							theErr;



	// Dispose of the tracking area
	for (theIter = mTrackingAreas.begin(); theIter != mTrackingAreas.end(); theIter++)
		{
		if (theIter->theID == theID)
			{
			// Clean up
			//
			// Our tracking areas will be destroyed automatically when the view is
			// destroyed, but if we're removing them before that point we need to
			// release them individually.
			CFSafeRelease(theIter->theShape);
			
			if (IsValid())
				{
				theErr = HIViewDisposeTrackingArea(theIter->trackArea);
				NN_ASSERT_NOERR(theErr);
				}
	...
	NN_LOG("Unknown tracking area: %d", theID);
}
...
//============================================================================
//      NHIView::GetTrackingArea : Get a tracking area.
//----------------------------------------------------------------------------
HIViewTrackingArea NHIView::GetTrackingArea(HIViewTrackingAreaID theID)
{	HIViewTrackingAreaListConstIterator		theIter;
	HIViewTrackingArea						theArea;



	// Validate our parameters
	NN_ASSERT(theID != kHIViewTrackingAreaNone);



	// Get the state we need
	memset(&theArea, 0x00, sizeof(theArea));



	// Get the tracking area
	for (theIter = mTrackingAreas.begin(); theIter != mTrackingAreas.end(); theIter++)
		{
		if (theIter->theID == theID)
			{
...
void NHIView::InitializeSubView(void)
{
	...
	if (!GetClassID().StartsWith("com.apple", kCFCompareNoCase))
		{
...
//============================================================================
//      NHIView::DisposeTrackingAreas : Dispose of the tracking areas.
//----------------------------------------------------------------------------
void NHIView::DisposeTrackingAreas(void)
{


	// Dispose of the tracking areas
	while (!mTrackingAreas.empty())
		DisposeTrackingArea(mTrackingAreas.front().theID);
}
...
//============================================================================
//      NHIView::UpdateTrackingAreas : Update the tracking areas.
//----------------------------------------------------------------------------
void NHIView::UpdateTrackingAreas(void)
{	EventTypeSpec	theEvent = { kEventClassMouse, kEventMouseMoved };
	...
	// Decide if we need a "within" handler
	//
	// Tracking areas can generate enter/exit events for a shape within a view (or
	// the entire view), but do not provide notification of mouse movements within
	// that shape.
	//
	// Nano provides kEventNControlTrackingAreaWithin, which can be used to listen
	// for mouse movements within a tracking area (rdar://3509896).
	//
	// In addition we must also generate TrackingAreaExited/Entered events for sheet
	// windows, as these are no longer generated on 10.5 (rdar://5813869).
	//
	// To handle these cases with minimal overhead, we install a mouse-moved event
	// handler on demand (removing it when it is no longer required).
	if (mTrackingAreas.empty())
		needHandler = false;
	else
		{
		needHandler = IsEventActive(kEventClassNControl, kEventNControlTrackingAreaWithin);
		if (!needHandler)
			{
			if (theWindow.IsValid() && (theWindow.GetWindowClass() == kSheetWindowClass))
				needHandler = (IsEventActive(kEventClassControl, kEventControlTrackingAreaEntered) ||
							   IsEventActive(kEventClassControl, kEventControlTrackingAreaExited));
			}
		}
	...
	if (needHandler)
		...
		if (mTrackingHandler == NULL)
			{
			theErr = InstallApplicationEventHandler(TrackingHandler, 1, &theEvent, this, &mTrackingHandler);
			NN_ASSERT_NOERR(theErr);
			}
		}
	else
		...
		if (mTrackingHandler != NULL)
			{
			theErr = RemoveEventHandler(mTrackingHandler);
			NN_ASSERT_NOERR(theErr);
...
//============================================================================
//      NHIView::TrackingHandler : Carbon Event handler.
//----------------------------------------------------------------------------
OSStatus NHIView::TrackingHandler(EventHandlerCallRef /*callRef*/, EventRef eventRef, void *userData)
{	NHIView								*thisPtr = (NHIView *) userData;
	NCarbonEvent						theEvent(eventRef), trackEvent;
	bool								isWithin, wasWithin;
	...
	// Validate our parameters and state
	NN_ASSERT(theEvent.IsClass(kEventClassMouse) && theEvent.IsKind(kEventMouseMoved));
	NN_ASSERT(!thisPtr->mTrackingAreas.empty());



	// Check for a hit on our window
	theWindow = theEvent.GetParameterHIWindow(kEventParamWindowRef);
	if (theWindow == NULL || theWindow != thisPtr->GetHIWindow())
		return(eventNotHandledErr);
	...
	// Get the state we need
	theView = NULL;
	theErr  = HIViewGetViewForMouseEvent(HIViewGetRoot(theWindow), eventRef, &theView);
	NN_ASSERT_NOERR(theErr);
	
	if (theView == NULL || theView != (HIViewRef) *thisPtr)
		thePoint = CGPointMake(-1.0f, -1.0f);
	else
		{
		thePoint = theEvent.GetMouseLocation();
		thePoint = NGeometryUtilities::Transform(thePoint, kHICoordSpace72DPIGlobal, NULL, kHICoordSpaceView, theView);
		}



	// Update the tracking areas
	for (theIter = thisPtr->mTrackingAreas.begin(); theIter != thisPtr->mTrackingAreas.end(); theIter++)
		{
		// Hit-test the shape
		if (theIter->theShape != NULL)
			isWithin = HIShapeContainsPoint(theIter->theShape,  &thePoint);
		else
			isWithin = CGRectContainsPoint(thisPtr->GetBounds(), thePoint);

		wasWithin         = theIter->isWithin;
		theIter->isWithin = isWithin;



		// Identify the event
		if (isWithin && !wasWithin)
			trackEvent = NCarbonEvent(kEventClassControl,  kEventControlTrackingAreaEntered);

		else if (!isWithin && wasWithin)
			trackEvent = NCarbonEvent(kEventClassControl,  kEventControlTrackingAreaExited);

		else if (isWithin)
			trackEvent = NCarbonEvent(kEventClassNControl, kEventNControlTrackingAreaWithin);
		
		else
			trackEvent = NCarbonEvent();
		...
		if (trackEvent.IsValid())
			{
			trackEvent.SetParameter(kEventParamHIViewTrackingArea, typeHIViewTrackingAreaRef, sizeof(theIter->trackArea), &theIter->trackArea);
			trackEvent.SetKeyModifiers(theEvent.GetKeyModifiers());
			trackEvent.SetMouseLocation(thePoint);

			trackEvent.SendToHIView(*thisPtr, kEventTargetDontPropagate);
			}
		}
