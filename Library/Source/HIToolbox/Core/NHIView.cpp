...
{	OSStatus	theErr;
	...
	// Request a redraw
	//
	// System controls automatically redraw when their enabled state changes,
	// however custom views must explicitly invalidate themselves.
	SetNeedsDisplay();
}
...
{	NHIView		*superView;
	...
	superView = NHIViewUtilities::GetHIView(hiView);
...
//----------------------------------------------------------------------------
void NHIView::SetNeedsDisplay(bool needsDisplay)
{	OSStatus	theErr;
...
DragActions NHIView::DoDragReceive(DragRef /*theDrag*/)
{
	...
	// Receive the drag
	//
	// Sub-classes should override to receive a drag, and return
	// the action with which the drag data was handled.
...
{	HIViewRef		contentView;
	...
	// be associated with the HIObjectRef and we just need to invoke InitializeView
	...
	// to ourselves (which will invoke InitializeView for us).
	if (GetHIObject() == NULL)
...
{	const EventTypeSpec	theEvents[] = { { kEventClassControl, kEventControlDragEnter          },
