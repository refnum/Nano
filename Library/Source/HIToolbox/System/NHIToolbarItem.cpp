...
{	const EventTypeSpec	theEvents[] = { { kEventClassToolbarItem, kEventToolbarItemCreateCustomView },
    ...
    theErr = HIToolbarItemCreate(mInfo.theIdentifier, mInfo.theAttributes, &theItem);
    ...
    if (mInfo.theCommand != kHICommandNone)
        SetCommandID(mInfo.theCommand);
    
    SetLabel(  NBundleString(mInfo.keyLabel));
    SetHelpTag(NBundleString(mInfo.keyHelpTagStd), false);
    SetHelpTag(NBundleString(mInfo.keyHelpTagExt), true);

    if (mInfo.resImage.IsNotEmpty())
        {
        if (NBundleResource(mInfo.resImage).Exists())
            SetImage(mInfo.resImage);
        
        else if (mInfo.resImage.GetSize() == 4)
            SetIcon(NIcon(kSystemIconsCreator, NTextUtilities::GetOSType(mInfo.resImage)));

        else
            SetIcon(NIcon(mInfo.resImage));
		}
...
{	OSStatus	theErr;
	...
	// Validate our parameters
	//
	// kHIToolbarItemBehaviourIsSelectable is used to enable automatic selection
	// of toolbar items (via a kEventToolbarGetSelectableIdentifiers event).
	//
	// As such, it conflicts with manual management of the selection state (as
	// performed by, e.g., NWindowPanels) and so we assert that the selection
	// state is not manipulated for these items.
	if (mInfo.theBehaviour & kHIToolbarItemBehaviourIsSelectable)
		{
		NN_ASSERT((  setThese & kHIToolbarItemSelected) == 0);
		NN_ASSERT((clearThese & kHIToolbarItemSelected) == 0);
		}
