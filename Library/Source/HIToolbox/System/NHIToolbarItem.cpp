...
{	const EventTypeSpec	theEvents[] = { { kEventClassToolbarItem, kEventToolbarItemCreateCustomView },
	...
	if (theInfo.resImage.IsNotEmpty())
		{
		if (NBundleResource(theInfo.resImage).Exists())
            SetImage(theInfo.resImage);
		
		else if (theInfo.resImage.GetSize() == 4)
			SetIcon(NIcon(kSystemIconsCreator, NTextUtilities::GetOSType(theInfo.resImage)));

		else
		    SetIcon(NIcon(theInfo.resImage));
		}
