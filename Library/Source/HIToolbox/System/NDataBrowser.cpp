...
// Misc
static const EventTime kUpdateDelay									= 0.75;
...
{	NDictionary		theDictionary;
	...
	// Reset the browser
	//
	// The current state is preserved around the reset, since we reset the
	// contents not user state such as sort order/column sizes.
	theErr = EncodeState(theDictionary);
	NN_ASSERT_NOERR(theErr);
	...
	if (theErr == noErr)
		{
		theErr = DecodeState(theDictionary);
		NN_ASSERT_NOERR(theErr);
		}
}
...
//============================================================================
//		NDataBrowser::Update : Update the browser.
//----------------------------------------------------------------------------
void NDataBrowser::Update(const NFunctor &theFunctor)
{


	// Perform the update
	PerformUpdate(theFunctor);
}
...
//============================================================================
//		NDataBrowser::IsUpdatingSelection : Is the selection being updated?
//----------------------------------------------------------------------------
bool NDataBrowser::IsUpdatingSelection(void) const
{


	// Get our state
	return(mUpdatingSelection);
}
...
//============================================================================
//		NDataBrowser::IsRemovingItems : Are items being removed?
//----------------------------------------------------------------------------
bool NDataBrowser::IsRemovingItems(void) const
{


	// Get our state
	return(mRemovingItems);
}
...
//============================================================================
//		NDataBrowser::GetDisclosureProperty : Get the disclosure property.
//----------------------------------------------------------------------------
DataBrowserPropertyID NDataBrowser::GetDisclosureProperty(bool *expandableRows) const
{	DataBrowserPropertyID	theProperty;
	Boolean					expandFlag;
	OSStatus				theErr;
	...
	// Get the disclosure property
	theProperty = kDataBrowserItemNoProperty;
	expandFlag  = false;
	theErr      = GetDataBrowserListViewDisclosureColumn(*this, &theProperty, &expandFlag);
	NN_ASSERT_NOERR(theErr);

	if (expandableRows != NULL)
		*expandableRows = expandFlag;
...
//============================================================================
//		NDataBrowser::SetDisclosureProperty : Set the disclosure property.
//----------------------------------------------------------------------------
void NDataBrowser::SetDisclosureProperty(DataBrowserPropertyID theProperty, bool expandableRows)
{	OSStatus	theErr;



	// Set the disclosure property
	theErr = SetDataBrowserListViewDisclosureColumn(*this, theProperty, expandableRows);
	NN_ASSERT_NOERR(theErr);
}
...
//============================================================================
//		NDataBrowser::EncodeState : Encode the view state.
//----------------------------------------------------------------------------
OSStatus NDataBrowser::EncodeState(NDictionary &theState) const
{	CFDictionaryRef		cfDictionary;
	...
	if (theErr == noErr)
		{
		theState.SetValueDictionary(kDBUserStateKey, cfDictionary);
...
//============================================================================
//		NDataBrowser::DecodeState : Decode the view state.
//----------------------------------------------------------------------------
OSStatus NDataBrowser::DecodeState(const NDictionary &theState)
{	NDictionary		theDictionary;
	OSStatus		theErr;
	...
	// Decode our state
	theDictionary = theState.GetValueDictionary(kDBUserStateKey);
	if (theDictionary.IsNotEmpty())
		{
...
{	EventTypeSpec	theEvents[] = { { kEventClassControl,	kEventControlSetFocusPart },
...
//============================================================================
//		NDataBrowser::DoEventControlSetFocusPart : Handle kEventControlSetFocusPart.
//----------------------------------------------------------------------------
OSStatus NDataBrowser::DoEventControlSetFocusPart(NCarbonEvent &theEvent)
{	bool				hasFocus;
	...
	if (mSource != NULL)
		...
		hasFocus = (thePart != kHIViewFocusNoPart);
		
		mSource->BrowserFocus(hasFocus);
		}
...
//============================================================================
//		NDataBrowser::PerformUpdate : Perform a safe update.
//----------------------------------------------------------------------------
void NDataBrowser::PerformUpdate(const NFunctor &theFunctor)
{


	// Reset our state
	//
	// If an update is pending when a second is submitted, the second one wins.
	mTimer.RemoveTimer();
	...
	// Perform the update
	//
	// The DataBrowser can not be updated while the user is scrolling through
	// it, or its live scroll proc will crash.
	//
	// We work around this by checking for the mouse button, and deferring
	// the update until we're clear of any scroll action.
	//
	// We use our own timer to manage the functor, to automatically cancel
	// the update if we are destroyed before the timer fires.
	if (theFunctor != NULL)
		{
		if (Button())
			mTimer.AddTimer(BindSelf(NDataBrowser::PerformUpdate, theFunctor), kUpdateDelay);
		else
			theFunctor();
		}
}
...
//----------------------------------------------------------------------------
Boolean NDataBrowser::DBAddDragItem(ControlRef browser, DragRef theDrag, DataBrowserItemID item, ItemReference *itemRef)
{	NDataBrowserSource		*theSource = GetSource(browser);
...
//----------------------------------------------------------------------------
Boolean NDataBrowser::DBAcceptDrag(ControlRef browser, DragRef theDrag, DataBrowserItemID item)
{	NDataBrowserSource		*theSource = GetSource(browser);
...
//----------------------------------------------------------------------------
Boolean NDataBrowser::DBReceiveDrag(ControlRef browser, DragRef theDrag, DataBrowserItemID item)
{	NDataBrowserSource		*theSource = GetSource(browser);
...
//----------------------------------------------------------------------------
void NDataBrowser::DBPostProcessDrag(ControlRef browser, DragRef theDrag, OSStatus trackDragResult)
{	NDataBrowserSource		*theSource = GetSource(browser);
