...
void NDataBrowserSource::SelectionRemove(void)
{
	...
	// Inform our listeners
	BroadcastMessage(kMsgBrowserSelectionRemove, this);
}
...
void NDataBrowserSource::SelectionAccept(void)
{
	...
	// Inform our listeners
	BroadcastMessage(kMsgBrowserSelectionAccept, this);
}
...
//============================================================================
//		NDataBrowserSource::BrowserFocus : The browser focus has changed.
//----------------------------------------------------------------------------
void NDataBrowserSource::BrowserFocus(bool hasFocus)
{


	// Inform our listeners
	BroadcastMessage(kMsgBrowserChangedFocus, (void *) hasFocus);
}
...
//----------------------------------------------------------------------------
bool NDataBrowserSource::CanDrag(DataBrowserItemID /*itemID*/, const NDrag &/*theDrag*/)
{


	// Can an item be dragged?
	//
	// Sub-classes should override if their items can be dragged, and add their
	// state to the drag (using itemID as the DragItemRef for AddDragItemFlavor).
	//
	// Note that CanDrag will only be called if at least one column in the browser
	// has the kDataBrowserListViewSelectionColumn property set (rdar://4025137).
	return(false);
}
...
//----------------------------------------------------------------------------
bool NDataBrowserSource::AcceptDrag(DataBrowserItemID /*itemID*/, const NDrag &/*theDrag*/)
{
...
//----------------------------------------------------------------------------
bool NDataBrowserSource::ReceiveDrag(DataBrowserItemID /*itemID*/, const NDrag &/*theDrag*/)
{
...
//----------------------------------------------------------------------------
void NDataBrowserSource::FinishDrag(OSStatus /*trackResult*/, const NDrag &/*theDrag*/)
{
...
//----------------------------------------------------------------------------
NDate NDataBrowserSource::GetItemDate(DataBrowserItemDataRef itemData) const
{	NDate			theValue;
	...
	if (theErr == noErr)
		theValue = NDate(NTimeUtilities::GetGregorianDate(theTime), kTimeZoneDefault);
...
//----------------------------------------------------------------------------
void NDataBrowserSource::SetItemDate(DataBrowserItemDataRef itemData, const NDate &theValue)
{	LongDateTime	theTime;
	OSStatus		theErr;
	...
	// Get the value
	theTime = NTimeUtilities::GetLongTime(theValue.GetAsGregorian());
...
//============================================================================
//		NDataBrowserSource::SetDataBrowserItemDataText : Set the text for an item.
//----------------------------------------------------------------------------
OSStatus NDataBrowserSource::SetDataBrowserItemDataText(DataBrowserItemDataRef itemData, const NString &theText, bool stripNewline)
{	NString		newText;
	OSStatus	theErr;
	...
	//
	// Work around rdar://5213064 - the DataBrowser does not truncate or clip items
	// whose text contains embedded newline characters.
	//
	// The truncation for the property is simply ignored, and the text is drawn as
	// a single line (overwriting any subsequent properties on this row).
	//
	// To work around this we map any newline characters to spaces.
	newText = theText;
	...
	// Set the text
	theErr = ::SetDataBrowserItemDataText(itemData, newText);
	NN_ASSERT_NOERR(theErr);
