/*	NAME:
	...
	DESCRIPTION:
		...
		implementation that an be customized as required.
...
{	EventTypeSpec		theEvents[] = {	{ kEventClassCommand,  kEventCommandProcess						},
	...
	SetOptions( kHIViewOptionWebViewUpdateWindowTitle | kHIViewOptionWebViewUpdateWindowIcon);
...
{	HIRect		theFrame, parentBounds;
	...
	// Get the state we need
	theFrame     = GetFrame();
	parentBounds = GetSuperView()->GetBounds();



	// Sync the web view frame
	theOrigin.x = theFrame.origin.x;
	theOrigin.y = parentBounds.size.height - CGRectGetMaxY(theFrame);
	
	[mWebView setFrameOrigin:theOrigin];
	[mWebView   setFrameSize:*((NSSize *) &theFrame.size)];
}
