...
{	NCGImage	theImage;
	...
	// Work around rdar://5570580.
	//
	// HIImageViewCopyImage always returns NULL on 10.5, so we double-check
	// a NULL result by fetching the (deprecated) tag value for the image.
	if (!theImage.IsValid())
		{
		GetControlData(kControlEntireControl, kHIImageViewImageTag, sizeof(cgImage), &cgImage);
		if (cgImage != NULL)
			theImage.Set(cgImage, false);
		}
...
{	HISize	theSize;
	...
	theErr = HIImageViewSetScaleToFit(*this, false);
	NN_ASSERT_NOERR(theErr);
...
{	NArray		theTypes;
	...
	if (mIsEditable)
		{
		theTypes.AppendValueString(kUTNTypeFileURL);
		theTypes.AppendValueString(kUTNTypeImage);
		}
...
//----------------------------------------------------------------------------
DragActions NHIImageView::DoDragReceive(const NDrag &theDrag)
{	NPasteboard		pasteBoard(theDrag);
	...
	if (theFile.IsValid())
		{
		if (theImage.Set(NCoreGraphicsUtilities::CreateImageFromFile(theFile)))
			{
			SetImage(theImage);
