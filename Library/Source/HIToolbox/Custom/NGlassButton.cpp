/*	NAME:
	...
	DESCRIPTION:
		...
		and override kEventControlDraw to customize the appearance.
...
// Colors
static const GlassButtonStyle kGlassButtonStyleActive			= { NColor(0x747474BB),
																	NColor(0xA9A9A9BB),
																	NColor(0x666666FF) };
...
{	NShadingSample				samplesBottom[] = { { 0.0f, kColorGreen }, { 0.5f, kColorYellow }, { 1.0f, kColorBlue } };
	...
	midX = minX + floorf(CGRectGetWidth(theRect) / 2.0f) + 1.0f;

	minY = CGRectGetMinY(theRect);
	maxY = CGRectGetMaxY(theRect);
	midY = minY + floorf(CGRectGetHeight(theRect) / 2.0f) + 1.0f;
	...
	cgContext.SaveState();
		...
		cgContext.AddRect(CGRectInset(theRect, 0, 1));
