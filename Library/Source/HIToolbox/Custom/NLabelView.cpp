...
static const CGShadowInfo kCrossShadow								= { NColor(0x00000011), 1.0f, CGSizeMake(0.0f, -1.0f) };

static const NColor       kCircleColor								= kColorWhite;
static const CGShadowInfo kCircleShadow								= { NColor(0x00000088), 1.75f, CGSizeMake(0.0f, -1.9f) };
...
{	NString		theFormat;
	...
	if (mPartNames.empty())
		{
		theFormat  = NBundleString(kStringLabelColor, "", kNanoStrings);
		mPartNames = NSystemUtilities::GetLabelNames();

		vector_push_front(mPartNames, NString(""));
		
		for (n = 1; n < kLabelPartCount; n++)
			{
			NN_ASSERT(mPartNames[n].IsNotEmpty());
			mPartNames[n].Format(theFormat, mPartNames[n]);
			}
		}
...
{	NCarbonEvent	hitTestEvent(kEventClassControl, kEventControlHitTest);
	...
	// Check our state
	if (GetSuperView() == NULL || !NHIWindow(GetHIWindow()).IsVisible())
		return(eventNotHandledErr);
	...
	if (mRollOverNew != mRollOver)
		...
		else if (!mTimer.HasTimer())
			mTimer.AddTimer(BindSelf(NLabelView::UpdateRollOver), kDelayRollOver);

		else
			mTimer.SetNextFireTime(kDelayRollOver);
		}
...
{	HIRect		theRect;
	...
	theRect.origin.x = floorf(thePoint.x - (theRect.size.width  / 2.0f));
	theRect.origin.y = floorf(thePoint.y - (theRect.size.height / 2.0f));
...
{	NShadingSample		theSamples[] = { { 0.0f, kColorRed }, { 1.0f, kColorBlue } };
	...
	// Get the state we need
	theRect.origin.x    = floorf(thePoint.x - (kGlyphWidth  / 2.0f));
	theRect.origin.y    = floorf(thePoint.y - (kGlyphHeight / 2.0f));
	...
	if (thePart == 1)
		...
		cgContext.SetShadow(     kCrossShadow);
	...
	// Draw a circle
		...
		cgContext.SetShadow(   kCircleShadow);
		cgContext.SetFillColor(kCircleColor);
