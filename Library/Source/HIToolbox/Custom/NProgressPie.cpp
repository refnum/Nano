...
{	float				theRadius, percentDone, angleCircle, angleStart, angleEnd;
	...
	if (NMathUtilities::AreEqual(percentDone, 0.0f))
		{
		cgContext.AddCircle(theCenter, theRadius - kBorderWidth);
		cgContext.FillPath();
		}

	else if (NMathUtilities::NotEqual(percentDone, 1.0f))
		{
