...
NHIView *NScrollableView::GetCanvasView(bool canCreate)
{
	...
	//
	// We need to enable embedding to embed the canvas view, and we turn on
	// embedding for the canvas view since the canvas-based scroll model is
	// typically used to embed HIViews within a scrollable view.
	//
	// Ideally canvas mode would be supported by HIScrollView, allowing it
	// to support both "view manages the size/origin" and "system moves your
	// view" modes of scrolling.
	//
	// Until then (rdar://4803121) we implement the latter ourselves, by
	// placing a canvas view within the scrolling view which is positioned
	// to reflect our the scrolling view's image state.
...
{	HISize		viewSize;
	...
	//
	// If the image is larger than the view bounds then the content fills the
	// entire view, and the content bounds are simply the image bounds in view
	// coordinates.
	//
	// If the image is smaller than the view bounds, it is allowed to "float"
	// within the view (currently centered).
	//
	...
	if (theRect.size.width < viewSize.width)
		theRect.origin.x = floor((viewSize.width  - theRect.size.width)  / 2.0f);

	if (theRect.size.height < viewSize.height)
		theRect.origin.y = floor((viewSize.height - theRect.size.height) / 2.0f);
