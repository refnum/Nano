...
//----------------------------------------------------------------------------
// Timers
static const NTimerID kTimerDrawContent								= 1000;
...
void NOpenGLView::SetFrameRate(EventTime theTime)
{
	...
	// Adjust the timer
	RemoveTimer(kTimerDrawContent);
	
	if (NMathUtilities::NotZero(mFrameRate))
		AddTimer(0.0f, mFrameRate, kTimerDrawContent);
}
...
//============================================================================
//		NOpenGLView::DoTimer : Handle a timer.
//----------------------------------------------------------------------------
void NOpenGLView::DoTimer(NTimerID theTimer, TimerMsg theMsg)
...
{	static float	sRotateBy = 0.0f;
	...
	//
	// To minimise flicker between the window being flushed to the screen and the
...
//============================================================================
//		NOpenGLView::SyncContextBounds : Synchronise the context bounds.
//----------------------------------------------------------------------------
...
{	NHIWindow		theWindow(GetHIWindow());
	...
	// Check our state
	if (mContext == NULL || theWindow.GetHIWindow() == NULL)
		return;
	...
	windowRect = theWindow.GetBounds();
	viewRect   = GetBounds();
