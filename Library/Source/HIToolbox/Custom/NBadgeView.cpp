...
static const CGShadowInfo kBadgeShadow								= { NColor(0.0f, 0.0f, 0.0f, 0.4f), 38.0f, CGSizeMake(0.0f, -10.0f) };
...
{	float				theScale, theInset;
	...
	shadowInfo = kBadgeShadow;

	shadowInfo.blur          = theScale / shadowInfo.blur;
	shadowInfo.offset.height = theScale / shadowInfo.offset.height;
...
{	float			edgeWidth, spikeWidth, spikeHeight;
	...
	// Get the state we need
	spikeWidth  = fabsf(kCornerPoints[kCornerCount-1].x - kCornerPoints[kCornerCount-2].x) * theScale * 2.0f;
	spikeHeight = fabsf(kCornerPoints[kCornerCount-1].y - kCornerPoints[kCornerCount-2].y) * theScale;
