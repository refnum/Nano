...
static const NColor			kBackgroundEdgeShadow =					 NColor(0.839, 0.839, 0.839, 0.75);
...
{	float		theValue;
	...
	switch (theTrack) {
		...
		case kMouseTrackingTimedOut:
			...
			SetPercentageValue(GET_CLAMPED_VALUE(theValue, 0.0f, 1.0f));
			break;
		}
...
{	NColor				colorTop, colorBottom;
	...
	// Draw the level
	theRect.size.width *= GetPercentageValue();
	maxX = CGRectGetMaxX(theRect);
...
{	UInt32				n, numCells, cellWidth, cellValue;
	...
	cellValue = (UInt32) (numCells * GetPercentageValue());
