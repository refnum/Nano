...
{	float			maxDistance;
	...
	theValue.theAngle    = (float) (atan2(-theOffset.y, theOffset.x) - M_PI_2);
	theValue.theDistance = sqrtf((theOffset.x * theOffset.x) + (theOffset.y * theOffset.y));

	theValue.theAngle    = (1.0f - (theValue.theAngle    / (float) (M_PI * 2.0)));
...
{	float		radAngle, theDistance;
	...
	radAngle    = (float) (((1.0f - theValue.theAngle) * (M_PI * 2.0)) + M_PI_2);
	theDistance = theValue.theDistance * GetMaxDistance(theRect);
		


	// Calculate the point
	thePoint.x = CGRectGetMidX(theRect) + (cosf(radAngle) *  theDistance);
	thePoint.y = CGRectGetMidY(theRect) + (sinf(radAngle) * -theDistance);
