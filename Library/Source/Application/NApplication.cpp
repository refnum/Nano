...
{	NString		nibName, resName;
	...
	nibName = mProperties.GetValueString(kAppNibFileKey);
	resName = mProperties.GetValueString(kAppNibMenuBarKey);
	...
	if (nibRef != NULL)
		{
		NIBUtilities::SetMenuBar(nibRef, resName);
...
{	OSStatus	theErr;
	...
	// Check the file
	//
	// By default we won't open the same file twice.
	if (theFile != NULL)
		{
...
{	NString				textButton;
	...
	textButton = cfBundle.GetString(kStringFileDialogOpen, "", kNanoStrings);
...
{	NString			aboutTitle, aboutButton;
	...
	// Get the state we need
	aboutTitle  = cfBundle.GetString(kStringAboutTitle,    "", kNanoStrings);
	aboutButton = cfBundle.GetString(kStringAboutHomePage, "", kNanoStrings);
