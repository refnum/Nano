...
//============================================================================
//		NWindowController::GetUndoManager : Get our undo manager.
//----------------------------------------------------------------------------
const NUndoManager *NWindowController::GetUndoManager(void) const
{
...
//		NWindowController::GetUndoManager : Get our undo manager.
//----------------------------------------------------------------------------
NUndoManager *NWindowController::GetUndoManager(void)
{
...
//============================================================================
//		NWindowController::RecordUndo : Record an undo action.
//----------------------------------------------------------------------------
void NWindowController::RecordUndo(const NFunctor	&theAction,
...
{	NString			theValue;
	...
	// Set the action name
	if (theName.IsNotEmpty())
		{
		theValue = stringsAreKeys ? NBundleString(theName, theName) : theName;
		mUndoManager.SetActionName(theValue);
		}
...
//============================================================================
//		NWindowController::TrackUndo : Track an undo group.
//----------------------------------------------------------------------------
void NWindowController::TrackUndo(bool beginTrack, bool canFlush)
{


	// Track the undo
	if (beginTrack)
		GetUndoManager()->BeginGroup();
	else
		GetUndoManager()->EndGroup(canFlush);
}
...
{	CommandStatus	theStatus;
	...
	switch (hiCmd.commandID) {
		...
		case kHICommandNToggleToolbar:
			...
			NMenuItem(hiCmd.source.menu.menuRef, hiCmd.commandID).SetText(GetToggleText());
			break;
