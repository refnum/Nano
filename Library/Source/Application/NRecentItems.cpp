...
//============================================================================
//		NRecentItems::GetSize : Get the size.
//----------------------------------------------------------------------------
...
{	RecentItemGroup					*theGroup;
	...
	vector_push_front(*theFiles, theItem);
...
{	NCGContext				cgContext(theEvent);
	...
	cgContext.SetOrientation(kHIThemeOrientationInverted);
...
{	UInt32				n, m, numGroups, numFiles;
	...
	for (n = 0; n < numGroups; n++)
		...
		for (m = 0; m < numFiles; m++)
			...
			if (theFile.theItem.GetFile().Exists())
