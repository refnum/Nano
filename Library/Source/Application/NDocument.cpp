...
static const NString kDocPrintStateKey					= "DocPrintState";
...
{	NString							theName, theExtension;
	...
	if (!HasFile() && mUntitledName.IsEmpty())
		...
		if (mUntitledCount > 1)
			mUntitledName.Format("%@ %d", mUntitledName, mUntitledCount);
		
		if (theExtension.IsNotEmpty())
			mUntitledName.Format("%@.%@", mUntitledName, theExtension);
		}
...
{	NCFBundle	appBundle;
	...
	//
	// Sub-classes can customize this behaviour by overriding these properties, but this
...
{	bool		hadSheet;
	...
	//
	// To allow sub-classes to process the command, or NWindowController to close
...
//				should implement EncodeState/DecodeState, and their
...
{	NDictionary		theState;
	...
	// Decode the document
	theState = theFile.GetProperties();
	theErr   = DecodeState(theState);
...
{	NDictionary		theState;
	...
	// Save the file
	theFile.SetProperties(theState);
...
//============================================================================
//		NDocument::EncodeState : Encode the document state.
//----------------------------------------------------------------------------
OSStatus NDocument::EncodeState(NDictionary &theState) const
{
	...
	theState.SetValueRect(      kDocWindowFrameKey, GetWindow()->GetFrame());
	theState.SetValueDictionary(kDocPrintStateKey,  mPrintDialog.GetProperties());
...
//============================================================================
//		NDocument::DecodeState : Decode the document state.
//----------------------------------------------------------------------------
OSStatus NDocument::DecodeState(const NDictionary &theState)
{
	...
	if (theState.HasKey(kDocWindowFrameKey))
		GetWindow()->SetFrame(theState.GetValueRect(kDocWindowFrameKey));

	if (theState.HasKey(kDocPrintStateKey))
		mPrintDialog.SetProperties(theState.GetValueDictionary(kDocPrintStateKey));
...
NHIView *NDocument::GetPrintView(void)
{
	...
	//
	// Sub-classes should override to return the view to print, if they are
