...
static const NString kDocWindowBoundsKey					= "DocWindowBounds";
...
{	EventTypeSpec		theEvents[] = {	{ kEventClassCommand,	kEventCommandUpdateStatus },
...
{	NString							theName, theExtension;
	...
	if (!HasFile() && mUntitledName.IsEmpty())
		...
		mUntitledName = cfBundle.GetString(kStringDocumentUntitled, "", kNanoStrings);
...
//============================================================================
//		NDocument::InitializeSelf : Initlaise ourselves
//----------------------------------------------------------------------------
...
{	NString				fileName, fileUTI, textButton;
	...
	// Choose the file
	textButton = cfBundle.GetString(kStringFileDialogSave, "", kNanoStrings);
...
OSStatus NDocument::EncodeProperties(NDictionary &theProperties)
{
	...
	theProperties.SetValueRect(kDocWindowBoundsKey, GetWindow()->GetBounds());
...
OSStatus NDocument::DecodeProperties(const NDictionary &theProperties)
{
	...
	//
	// Sub-classes should override to encode their properties, then call through.
	if (theProperties.HasKey(kDocWindowBoundsKey))
		GetWindow()->SetBounds(theProperties.GetValueRect(kDocWindowBoundsKey));
...
{	OSType			fileType, fileCreator;
	...
	if (fileIcon.IsNotEmpty())
		{
		theFile = cfBundle.GetResource(fileIcon, "icns");
		if (theFile.IsFile())
