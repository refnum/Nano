...
//============================================================================
//		NUndoManager::GetCommandString : Get a command string.
//----------------------------------------------------------------------------
NString NUndoManager::GetCommandString(UInt32 cmdID, CommandStatus cmdStatus, const NString &cmdName)
{	NString		theValue, keyNone, keyGeneric, keyCommand;



	// Validate our parameters
	NN_ASSERT(cmdID == kHICommandUndo || cmdID == kHICommandRedo);



	// Select the keys
	if (cmdID == kHICommandUndo)
		{
...
{	CommandStatus	theStatus;
	...
	switch (hiCmd.commandID) {
		case kHICommandUndo:
			...
			theValue  = GetCommandString(hiCmd.commandID, theStatus, GetUndoName());
			break;

		case kHICommandRedo:
			theStatus = EnableIf(CanRedo());
			theValue  = GetCommandString(hiCmd.commandID, theStatus, GetRedoName());
			break;
