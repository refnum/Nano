...
{	float			timeDelta;
	...
	// Change the panel
	theView = NHIViewUtilities::GetView(NIBUtilities::CreateView(mNib, thePanel));
	if (theView != NULL)
		{
		...
		theView->SetLayoutInfo(NHIViewUtilities::GetLayoutInfo(kBindAllSides));
	...
	if (mFadeOld != NULL && mFadeNew != NULL)
		{
		mFadeAlpha = 0.0f;
		mFadeDelta = 1.0f / kTransitionSteps;
		timeDelta  = ((float) mTransitionTime) / kTransitionSteps;
		
		mFadeOld->SetAlpha(1.0f);
		mFadeNew->SetAlpha(0.0f);
		mContainer->SetVisible(false);

		mTimer.AddTimer(BindSelf(NPanelController::TransitionPanel), 0.0f, timeDelta);
		while (mTimer.HasTimer())
			{
...
//============================================================================
//		NPanelController::TransitionPanel : Transition between two panels.
//----------------------------------------------------------------------------
#pragma mark -
void NPanelController::TransitionPanel(void)
{
