...
{	NCFBundle	appBundle;
	...
	mInfo->ReplaceTextValue(kTokenAppName,    theInfo.GetValueString(kNanoAppNameKey));
	mInfo->ReplaceTextValue(kTokenVersionOld, theInfo.GetValueString(kNanoVersionOldKey));
	mInfo->ReplaceTextValue(kTokenVersionNew, theInfo.GetValueString(kNanoVersionNewKey));
...
{	NCFBundle		appBundle;
	...
	theURL.Format(theURL, theInfo.GetValueString(kNanoVersionOldKey).GetUTF8());
...
{	NString		theStatus, appName, appVersion;
	...
	theStatus.Format(theStatus, appName, appVersion);
...
{	NString		theStatus;
	...
	theStatus.Format(theStatus, mUpdateFile.GetFileName());
...
{	NXMLTree		tmpNode, theChild;
	...
	if (!theItems.IsEmpty())
		...
		theItem = theItems.GetValueDictionary(theItems.GetSize() - 1);
...
{	UInt32			n, numChildren;
	...
	for (n = 0; n < numChildren; n++)
		...
		if (theChild.IsElement("enclosure"))
			...
			theItem.SetValueDate(  theChild.GetData(), theChild.GetElementValue());

		else if (theChild.IsElement())
			theItem.SetValueString(theChild.GetData(), theChild.GetElementValue());
		}
...
{	NSUWindowInfo	*theWindow;
	...
	switch (theCmd) {
		...
		case kCmdUpdateInstall:
			if (theInfo.GetValueString(kItemSelfInstallKey).Contains("true", kCFCompareNoCase))
...
{	NString			textMajor, textMinor;
	...
	switch (theErr) {
		...
		default:
			...
			textMinor.Format(textMinor, theErr);
			break;
		}
...
{	NString		theValue;
	...
	if (verName.IsEmpty())
		...
		theValue.Format("%@ (%@)", verName, verBuild);
